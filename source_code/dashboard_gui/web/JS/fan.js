...
setInterval(async () => {
  ...
  console.log(data);
  if (data !== null) {
    document.querySelector("#fanToggle").classList =
      data["fan-device"]["power"] == 0
    ...
    document.querySelector("#fan-duty-cycle").innerText =
      data["fan-device"]["duty_cycle"];
    ...
    document.querySelector("#statusLabel").innerText =
      data["fan-device"]["status"]["isOnline"] === 0 ? "OFFLINE" : "ONLINE";
    document.querySelector("#statusLabel").style.color =
      data["fan-device"]["status"]["isOnline"] === 0 ? "red" : "green";
  }
}, 500);
